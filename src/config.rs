...
/// 不停发请求」。取最近一次核对过的发行版（`@openai/codex` 0.156.0，npm `latest`，
/// 2026-09-23 核对）。
///
...
/// `gpt-6-sol`/`gpt-6-luna` 又是一轮：本机 0.155.0 拉到了，coban 报 0.153.4 拉不到。
...
/// 拼出来的身份才自洽——UA 说 0.156.0 而 `version` 说别的（或干脆不带），是没有哪个真实
/// 客户端会产生的形态。
