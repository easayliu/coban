    ...
    <div className="space-y-5">
      ...
      >
        <ChoiceSetting
          ...
          description={t(
            '默认原样透传，不动来访客户端报的东西。originator 这个头跟着这个设置一起走：透传档连它一起透传（来访没带才补上官方 codex CLI 那份），改写档连它一起收敛——两个头说的是同一件事「谁在发」，分开处理就会拼出「UA 说 Codex Desktop、originator 说 codex CLI」这种谁都产生不出来的组合。要不要收敛取决于你的接入方是什么：codex CLI 与 Codex Desktop 这类官方前端自带一份完整身份，透传就是对的——Desktop 的请求体里还带着 client_metadata 这种 CLI 不发的字段，只收敛头反而把体落单；而 OpenAI SDK、各种翻译类客户端压根不发 originator，只能拿到补上的 CLI 那份，于是造出「originator 说 codex CLI、UA 说 OpenAI/Python」这种官方客户端产生不出来的组合，走 /v1/chat/completions 那条路（请求体会被翻成 Responses 形状）更是必然如此——这类接入方存在时该开到「只改写不像官方客户端的」，来访确实是 codex CLI 的照旧透传（它报的版本可能比 coban 写死的更新）。「一律改写」留给确定只有翻译类客户端接入的场景。改写的目标值按账号派生，一个号一台稳定的机器、跨重启不变——不是整池共用一条 UA，那本身就是一簇可关联的指纹；改写时还会把 x-stainless-* 这类 SDK 留痕头一起清掉。三档都不管「来访压根没报 UA」那一格：那时一律补上该账号派生的那份，一个不带 UA 的客户端拿着订阅 token 打上游最显眼。账号页里「来访客户端」那一列显示的始终是客户端自报的原值，不受这个设置影响。',
            'Passes the caller’s UA through untouched by default. The originator header follows this setting: passed through with the UA (falling back to the official codex CLI value when the caller sends none), or converged along with it. Both headers say the same thing — who is calling — so handling them separately produces combinations nobody can make, such as “UA says Codex Desktop, originator says codex CLI”. Whether to converge depends on what connects to you: the codex CLI and Codex Desktop carry a complete identity of their own, so passing them through is right — Desktop also sends client_metadata in the body, a field the CLI never sends, so converging only the headers leaves the body behind. OpenAI SDKs and translated clients send no originator at all and can only get the CLI fallback, which makes upstream see “originator says codex CLI, UA says OpenAI/Python” — a combination the official client can never produce, and the /v1/chat/completions path (whose body is translated into Responses shape) is always like that. With such callers, switch to rewriting only what does not look official; a caller that really is the codex CLI still passes through, since its version may be newer than the one coban pins. “Always rewrite” is for setups where only translated clients connect. The replacement is derived per account, so each account is one stable machine across restarts — not one shared UA for the whole pool, which is a correlatable fingerprint in itself; rewriting also drops SDK trace headers such as x-stainless-*. None of the three settings covers a caller that sends no UA at all: that always gets the account’s derived UA, because a client with no UA holding a subscription token is the most conspicuous thing there is. The “Client” column on the accounts page always shows what the caller reported, regardless of this setting.',
          )}
